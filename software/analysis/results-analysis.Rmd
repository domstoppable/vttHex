...
```{r training-perf-by-level, echo = FALSE}
    ...
    lmer0 <- lmerTest::lmer(
        PercentageCorrect ~ 1 + (1|Game/Level) + (1|PID),
    ...
    lmer1 <- lmerTest::lmer(
        PercentageCorrect ~ p_l_progression + (p_l_progression|Game) + (p_l_progression|PID),
        data = level_attempt_progression_summary,
        weights = TrialCount,
        REML = FALSE
    )
    ...
    lmer2 <- lmerTest::lmer(
        PercentageCorrect ~ p_l_progression + (p_l_progression|Game/Level) + (p_l_progression|PID),
        data = level_attempt_progression_summary,
        weights = TrialCount,
        REML = FALSE
    )
    ...
    plot_values <- ggeffects::ggpredict(lmer2, "p_l_progression")

    ggplot2::ggplot(plot_values, ggplot2::aes(x = x, y = predicted)) +
        ggplot2::geom_line() +
        ggplot2::geom_ribbon(ggplot2::aes(ymin = conf.low, ymax = conf.high), alpha = .1) +
        ggplot2::geom_point(
            mapping = ggplot2::aes(
                x = p_l_progression,
                y = PercentageCorrect,
