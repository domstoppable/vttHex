...
training_data$trainingDayIdx <- ifelse(is.na(training_data$timedelta), 1, as.integer(training_data$timedelta > 3600))
training_data$trainingDayIdx <- cumsum(training_data$trainingDay)
...
write.csv(exposure_counts_by_stimulus, 'exposure_counts_by_stimulus.csv')
...
trainStim_exposure_counts_by_pid <- data.frame(dplyr::summarise(
    ...
    totalTrainStimExposuresForPID = sum(Ok)
))

write.csv(exposure_counts_by_phrase, 'exposure_counts_by_phrase.csv')
...
pronounciation_counts <- data.frame(dplyr::summarise(
    dplyr::group_by(exposure_counts_by_phoneme, phrase),
...
write.csv(exposure_counts_by_phoneme, 'exposure_counts_by_phoneme_0.csv')
...
write.csv(p_exposures, 'exposure_counts_by_phoneme.csv')
...
training_report <- data.frame(dplyr::summarise(
    ...
    levelAttempts = nrow(dplyr::filter(stims_per_level, pid==pid)),
    dateRange = as.integer(max(datetime) - min(datetime)),
    totalPhraseStimExposures = sum(Ok),
    meanTrainingDayDurationM = mean(dplyr::filter(training_day_summary, pid==pid)$durationInM)
))
...
load_files <- function(file_pattern, create_correct_column = TRUE) {
    ...
    df$totalTrainStimExposuresForPID <- ifelse(df$condition == "Pre-test", 0, df$totalTrainStimExposuresForPID)
...
phoneSummary <- dplyr::summarise(
    dplyr::group_by(phoneDF, pid, phoneType),
...
phone_m0 <- phone_model(correct ~ 1 + (wordExposures|pid))
phone_m1 <- phone_model(correct ~ wordExposures + (wordExposures|pid))
phone_m2 <- phone_model(correct ~ wordExposures + phoneType + (wordExposures|pid))
phone_m3 <- phone_model(correct ~ wordExposures*phoneType + (wordExposures|pid))
...
prosodyDF <- data.frame(dplyr::summarise(
    ...
    totalTrainStimExposuresForPID = mean(totalTrainStimExposuresForPID),
...
prosodySummary <- dplyr::summarise(
    ...
    correctPortion = 100*mean(portionCorrect),
    se = 100*sd(portionCorrect)/sqrt(length(portionCorrect)),
    totalTrainStimExposuresForPID = mean(totalTrainStimExposuresForPID)
)

ggplot2::ggplot(prosodySummary, ggplot2::aes(x=totalTrainStimExposuresForPID, y=correctPortion, shape=prosodyType, group=prosodyType, color=prosodyType)) +
#    ggplot2::facet_wrap(facets = ggplot2::vars(pid)) +
...
#
#focus_m0 = focus_model(correct ~ 1 + (totalTrainStimExposuresForPID | pid))
#focus_m1 = focus_model(correct ~ totalTrainStimExposuresForPID + (totalTrainStimExposuresForPID | pid))
#focus_m2 = focus_model(correct ~ totalTrainStimExposuresForPID + (totalTrainStimExposuresForPID | pid) + (1 | sentenceID))

focus_model <- function(formula) lme(formula = formula, data = focusDF)
lme4::lmer(portionCorrect ~ 1 + (totalTrainStimExposuresForPID|pid), data = focusDF)
#
#focus_m0 = focus_model(correct ~ 1 + (totalTrainStimExposuresForPID | pid))
#focus_m1 = focus_model(correct ~ totalTrainStimExposuresForPID + (totalTrainStimExposuresForPID | pid))
#focus_m2 = focus_model(correct ~ totalTrainStimExposuresForPID + (totalTrainStimExposuresForPID | pid) + (1 | sentenceID))
...
ggplot2::ggplot(integrationSummary, ggplot2::aes(x=totalTrainStimExposuresForPID, y=correctPortion)) +
#    ggplot2::facet_wrap(facets = ggplot2::vars(pid)) +
...
ggplot2::ggsave(plot=ggplot2::last_plot(), file="../plots/integration-good-speakers.png", width=6, height=6)
```
...
# checkout out roz's pilot data
# maybe she did better on the ones she had the most exposure to?
phoneDF <- load_files("_phonemes_")
...
exposure_counts_by_phoneType <- data.frame(
    dplyr::summarise(
        dplyr::group_by(test_df, pid, condition, test_phone, phone_type),
        phonemeExposures = mean(phonemeExposures),
    )
)
exposure_counts_by_phoneType <- data.frame(
    dplyr::summarise(
        dplyr::group_by(exposure_counts_by_phoneType, pid, phone_type, condition),
        phonemeExposures = sum(phonemeExposures),
    )
)
...
test_summary <- data.frame(
    dplyr::summarise(
        dplyr::group_by(test_df, condition, pid, phone_type),
        correctPortion = 100*mean(correct),
        se = 100*sd(correct)/sqrt(length(correct))
    )
)
test_summary <- merge(test_summary, exposure_counts_by_phoneType, by=c('condition', 'pid', 'phone_type'))

ggplot2::ggplot(test_summary, ggplot2::aes(x=phonemeExposures, y=correctPortion, shape=phone_type, group=phone_type, color=phone_type)) +
#    ggplot2::facet_wrap(facets = ggplot2::vars(pid)) +
...
pilot_phone_m1 <- lm(correctPortion ~ phonemeExposures + phone_type, data=test_summary)
summary(pilot_phone_m1)

#test_summary_plot_data <- data.frame(dplyr::summarise(
#    dplyr::group_by(test_summary, correctPortion, phonemeExposures, phone_type),
...
ggplot2::ggplot(test_summary, ggplot2::aes(x=phonemeExposures, y=correctPortion, label=test_phone, color=phone_type)) +
    ggplot2::geom_hline(yintercept=100/9, linetype='dashed', color='gray') +
    ...
    ggplot2::labs(
        ...
        x = "Phoneme Exposures",
        y = "Percentage Correct",
        title = "Phoneme Identification - Training vs Performance (post-test only)"
    ) +
    ggplot2::theme_classic(base_size=10) +
    ggplot2::theme(legend.position = c(0.85, 0.85))
...
pilot_phone_gm0p <- glm(correct ~ phonemeExposures + phone_type, data=pre_test_only, family='binomial')
summary(pilot_phone_gm0p)

test_summary <- data.frame(
    dplyr::summarise(
        dplyr::group_by(pre_test_only, pid, phone_type, test_phone),
        correctPortion = 100*mean(correct),
        phonemeExposures = mean(phonemeExposures),
        se = 100*sd(correct)/sqrt(length(correct))
    )
...
ggplot2::ggplot(test_summary_plot_data, ggplot2::aes(x=phonemeExposures, y=correctPortion, label=test_phone, color=phone_type)) +
    ggplot2::geom_hline(yintercept=100/9, linetype='dashed', color='gray') +
    ggplot2::geom_line(ggplot2::aes(y=fitLine$fit), color='black') +
    ggplot2::geom_ribbon(ggplot2::aes(ymin=fitLine$lwr, ymax=fitLine$upr), alpha = .15, color='gray') +
    ggplot2::geom_point(show.legend = FALSE) +
...
test_summary <- data.frame(
    dplyr::summarise(
        dplyr::group_by(test_df, test_phone, phone_type, condition),
        correctPortion = 100*mean(correct),
        phonemeExposures = mean(phonemeExposures)
    )
)
test_summary_plot_data <- data.frame(dplyr::summarise(
    dplyr::group_by(test_summary, correctPortion, phonemeExposures, test_phone, phone_type, condition),
    overlaps = dplyr::n()
))
...
# Post-test prosody
```{r}
focus_m0 = lm(portionCorrect ~ totalTrainStimExposuresForPID, data=focusDF)
summary(focus_m0)

phrase_m0 = lm(portionCorrect ~ totalTrainStimExposuresForPID, data=phraseDF)
summary(phrase_m0)
...
write.csv(phone_distribution_delta, 'phone_distribution_delta.csv')
```
